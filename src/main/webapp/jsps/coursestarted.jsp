...
<body>
    <section id="success">
        <div>
            <c:if test="${success}">
    			<span style="color: green;">Course started successfully</span>
    		</c:if>
    		<c:if test="${nullPointer}">
    			<span style="color: red;">No course selected</span>
    		</c:if>
    		<c:if test="${noOption}">
    			<span style="color: red;">Student is attending all courses</span>
    		</c:if>
    		<c:if test="${SQLError}">
